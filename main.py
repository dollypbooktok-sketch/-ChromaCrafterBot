...
def get_color_preview(hex_color: str) -> str:
    """Create a visual preview of a color."""
    return "█" * 20
...
def format_palette(color: str) -> str:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    welcome = (
        f"🎨 **Welcome to ChromaCrafterBot, {user.first_name}!**\n\n"
        "I'm your color crafting assistant. Here's what I can do:\n\n"
...
async def triadic_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle the /triadic command."""
    args = context.args
    
    if not args:
        await update.message.reply_text(
            "🔺 Please provide a hex color!\n"
            "Example: `/triadic #FF5733`"
        )
        return
    ...
    colors = get_triadic_colors(color)
    
    message = f"🔺 **Triadic Colors**\n\nOriginal: `{color}`\n\n"
    for i, c in enumerate(colors, 1):
        r, g, b = hex_to_rgb(c)
        message += f"Color {i}: `{c}` - RGB({r}, {g}, {b})\n"
        ...
        try:
            ...
            if len(parts) == 3:
                ...
                if all(0 <= x <= 255 for x in (r, g, b)):
                    ...
                    keyboard = [
                        [
                            InlineKeyboardButton("🔄 Complement", callback_data=f"comp_{hex_color}"),
                            InlineKeyboardButton("🎨 Palette", callback_data=f"palette_{hex_color}"),
                        ]
                    ]
                    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Help
        ...
        await query.edit_message_text(
            ...
            "• `/analogous [color] [count]` - Analogous\n"
...
def main() -> None:
    """Start the bot."""
    try:
        logger.info("🚀 Starting ChromaCrafterBot...")
        ...
        app.add_handler(CommandHandler("palette", palette_command))
        app.add_handler(CommandHandler("complementary", complementary_command))
        app.add_handler(CommandHandler("analogous", analogous_command))
        app.add_handler(CommandHandler("triadic", triadic_command))
